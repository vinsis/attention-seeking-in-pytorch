...
content_based_attention_general = nn.Linear(encoder_hidden_size*2, encoder_hidden_size*2)

trainable_parameters = [{'params': net.parameters()} for net in [embedding, encoder, decoder, decoder_output_to_sequence_length, content_based_attention_general]]
...
content_based_attention_general.to(device)
...
def train():
    ...
    for index, random_sequence in enumerate(loader):
        ...
        for time in range(sequence_length):
            ...
            transformed_encoder_outputs = content_based_attention_general(encoder_outputs)
            similarity_with_hidden_state = torch.bmm(decoder_input_h, transformed_encoder_outputs.transpose(1,2))
            encoder_output_weights = F.softmax(similarity_with_hidden_state, dim=2)
