...
def train():
    ...
    for index, random_sequence in enumerate(loader):
        ...
        for time in range(sequence_length):
            ...
            encoder_output_weights = F.softmax(similarity_with_reference_vector, dim=2) # size: 1,1,sequence_length
            encoder_output_with_highest_probability = encoder_outputs[:, encoder_output_weights.argmax(), :].unsqueeze(1)
            # weighted_sum_of_encoder_outputs = torch.bmm(encoder_output_weights, encoder_outputs)
            # attention ends here
            decoder_output_at_time_t, (decoder_h, decoder_c) = decoder(decoder_input, (decoder_input_h, encoder_output_with_highest_probability))
